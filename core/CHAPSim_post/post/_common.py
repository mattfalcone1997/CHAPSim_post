...
class Common(ABC):
    ...
    # def _set_vtk_cell_arrays_from_datastruct(self,grid,name,array):
        
    
    # def _set_vtk_cell_array_from_attr(self,grid,name):
    #     if not hasattr(self,name):
    #         msg = "%s is not an attribute of the class %s"%(name,self.__class__)
    #         raise AttributeError(msg)
    #     data_struct = getattr(self,name)
    #     if not isinstance(data_struct,cd.datastruct):
    #         msg = "The attribute must be an instance of datastruct"
    #         raise TypeError(msg)
    #     dim = data_struct.keys()
    #     arr_size= data_struct.values[0].size
    #     grid.cell_arrays[name] = data_struct.values.reshape((dim,arr_size)).T
    
    # def _create_vtk3D_grid(self):
    #     x_coords = self._meta_data.Coord_ND_DF['x']
    #     y_coords = self._meta_data.Coord_ND_DF['y']
    #     z_coords = self._meta_data.Coord_ND_DF['z']
    ...
    #     Z = np.zeros_like(X)
    #     grid = pyvista.StructuredGrid(Y,X,Z)
    #     return grid
    ...
    # @abstractmethod
    # def _to_vtk(self,file_name,mode,attr_list):
    #     if not hasattr(self,"CoordDF"):
    #         msg = "Instance must have attribute named CoordDF"
    #         raise AttributeError(msg)
...
class common3D(Common):
    ...
    def plot_vector(self,ProcessDF,avg_data,slice,ax_val,PhyTime=None,y_mode='half_channel',spacing=(1,1),scaling=1,x_split_list=None,fig=None,ax=None,quiver_kw=None,**kwargs):
        ...
        if coord == 'y':
            ...
            ax_val = indexing.true_coords_from_coords(self.CoordDF,coord,ax_val)

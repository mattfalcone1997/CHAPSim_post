...
class CHAPSim_autocov_io(CHAPSim_autocov_base):
    ...
    def _autocov_calc_z(self,fluct1,fluct2,NCL1,NCL2,NCL3,max_z_step):
        ...
        if cp.rcParams['autocorr_mode'] in (1,2):    
            if cp.rcParams['autocorr_mode'] == 1:
                from CHAPSim_post.src._f90_ext_base import autocov_calc_z
                R_z = np.zeros((max_z_step,NCL2,NCL1),order='F')
            else:
                if cp.rcParams['dtype'] == np.float64:
                    from CHAPSim_post.src._cy_ext64_base import autocov_calc_z
                elif cp.rcParams['dtype'] == np.float32:
                    from CHAPSim_post.src._cy_ext32_base import autocov_calc_z
    ...
    def _autocov_numba_z(self,fluct1,fluct2,NCL1,NCL2,NCL3,max_z_step):
    ...
    def _autocov_calc_x(self,fluct1,fluct2,NCL1,NCL2,NCL3,max_x_step):
        if cp.rcParams['autocorr_mode'] in (1,2):    
            if cp.rcParams['autocorr_mode'] == 1:
                from CHAPSim_post.src._f90_ext_base import autocov_calc_x
                R_x = np.zeros((max_x_step,NCL2,NCL1-max_x_step),order='F')
            else:
                if cp.rcParams['dtype'] == np.float64:
                    from CHAPSim_post.src._cy_ext64_base import autocov_calc_x
                elif cp.rcParams['dtype'] == np.float32:
                    from CHAPSim_post.src._cy_ext32_base import autocov_calc_x

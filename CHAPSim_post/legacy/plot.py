...
from CHAPSim_post.legacy.utils import misc_utils
...
def create_fig_ax_with_squeeze(fig=None,ax=None,**kwargs):
    ...
    if fig is None:
        ...
        AxesClass = mpl.projections.get_projection_class('AxesCHAPSim') 
        AxesClass3D = mpl.projections.get_projection_class('Axes3DCHAPSim') 

        if not isinstance(ax,(AxesClass, AxesClass3D)):
